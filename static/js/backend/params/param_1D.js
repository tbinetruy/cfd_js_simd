import { getSolver, getEqn, getDiscretization, getBC_type, getIC } from "../app_config.js"

export const compute_param_1D = {
	createPDE: ({ discretizationType, t, nx, dt, L, c, nu, IC, BC, solver }) => {
		return {
			nt: t / dt,
			discretization: getDiscretization[discretizationType],
			nx,
			dt,
			dx: L / (nx - 1),
			IC,
